...
import uz.apextech.service.PartQueryService;
import uz.apextech.service.PartService;
...
/**
 * REST controller for managing {@link uz.apextech.domain.Part}.
 */
@RestController
@RequestMapping("/api")
public class PartResource {
    ...
    @CrossOrigin("http://localhost:56317/")
    @GetMapping("/part/{themeId}")
    public ResponseEntity<List<PartDTO>> getPartOfTheme(@PathVariable Long themeId) {
        log.debug("REST request to get Part by themeId : {}", themeId);
        List<PartDTO> parts = partService.findAllByTheme(themeId);
